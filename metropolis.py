...
def metropolis(steps,desired_dist):
    ...
    for i in range(steps):
        ...
        if (desired_dist(x_next)/desired_dist(x[i]))>=1:
            ...
            if np.random.rand()<=(desired_dist(x_next)/desired_dist(x[i])):
                x[i+1]=x_next
                accepted+=1
            else:
                x[i+1]=(x[i])
...
steps=100000   
[x,ave_x,var_x,acceptance_fraction]=metropolis(steps,gaussian_dist)
t=range(steps)
